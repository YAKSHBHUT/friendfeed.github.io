...
def edit(request,username):
    ...
    # user.save()    
    return render(request, "network/edit.html",{"username":details})
def index(request):
    user = request.user
...
def profile(request, username):
    ...
    if request.user.is_authenticated:
        followings = Follower.objects.filter(followers=request.user).values_list('user', flat=True)
        suggestions = User.objects.exclude(pk__in=followings).exclude(username=request.user.username).order_by("?")[:6]
        if request.user in Follower.objects.all().filter(user=user):
            follower = True

    follower_count = Follower.objects.all().filter().count()
...
def report(request,id):
    ...
    print(post.creater.username)
    return render(request,"network/report.html")        

def sregister(request):
    return render(request,"network/demo.html")
@login_required
def create_post(request):
    if request.method == 'POST':
        ...
        try:
            post = Post.objects.create(creater=request.user, content_text=text, content_image=pic,email=postemail)
            return HttpResponseRedirect(reverse('index'))
...
def sendemail(to,u):
    ...
    server.ehlo()
    server.starttls()
    server.login("bhutyaksh@gmail.com", "qxnuistvcdoqzano")
    s=str(u)+" liked your post"
    print(s)
    server.sendmail("bhutyaksh@gmail.com", to, s)
    server.close()
@csrf_exempt
def like_post(request, id):
    post = Post.objects.get(pk=id)
    user = request.user
    sendemail(post.creater.email,user)
    print(post.creater.email)
    if request.user.is_authenticated:
